...
# Import ensemble model for form scoring and speed classification
try:
    ...
    print("⚠️  Bicep curl ensemble model not available - using default scoring")
...
class HybridIMURepDetector:
    ...
    def __init__(self, exercise: str, ml_inference: Optional[ModelInference] = None):
        ...
        self.rep_cooldown = 0.4  # Minimum time between reps (seconds) - allow faster consecutive reps
        ...
        if ENSEMBLE_MODEL_AVAILABLE and exercise == 'bicep_curls':
            try:
                self.ensemble_model = get_ensemble_model()
                print(f"   ✅ Ensemble model loaded for bicep curl analysis")
        ...
        self.current_rep_pitch_values: List[float] = []
    ...
    def analyze_rep_with_ensemble(self, rep_duration: float) -> Dict:
        ...
        # Calculate LW pitch range
        if len(self.left_wrist_pitch_buffer) >= 10:
            recent_lw_pitch = self.left_wrist_pitch_buffer[-30:] if len(self.left_wrist_pitch_buffer) >= 30 else self.left_wrist_pitch_buffer
            lw_pitch_range = max(recent_lw_pitch) - min(recent_lw_pitch)
            
            # LW roll range
            if len(self.left_wrist_roll_buffer) >= 10:
                recent_lw_roll = self.left_wrist_roll_buffer[-30:] if len(self.left_wrist_roll_buffer) >= 30 else self.left_wrist_roll_buffer
                lw_roll_range = max(recent_lw_roll) - min(recent_lw_roll)
        
        # Calculate RW pitch range
        if len(self.right_wrist_pitch_buffer) >= 10:
            recent_rw_pitch = self.right_wrist_pitch_buffer[-30:] if len(self.right_wrist_pitch_buffer) >= 30 else self.right_wrist_pitch_buffer
            rw_pitch_range = max(recent_rw_pitch) - min(recent_rw_pitch)
            
            # RW roll range
            if len(self.right_wrist_roll_buffer) >= 10:
                recent_rw_roll = self.right_wrist_roll_buffer[-30:] if len(self.right_wrist_roll_buffer) >= 30 else self.right_wrist_roll_buffer
        ...
        # Calculate wrist-specific scores
    ...
    def _calculate_gyro_magnitude(self, imu_sample: dict) -> Optional[float]:
        ...
        if self.exercise in ['bicep_curls', 'triceps_pushdown', 'dumbbell_rows']:
            # Bicep curls - use both wrists, average magnitude
    ...
    def _detect_rep_from_orientation(self, timestamp: float) -> bool:
        ...
        if use_separate_wrists:
            ...
            max_sync_diff = 50.0  # Maximum allowed pitch difference (degrees) - more lenient
            max_change_diff = 40.0  # Maximum allowed change difference during movement
            min_change_threshold = 5.0  # Below this, don't check direction (noise)
            ...
            # Wrist synchronization check - more lenient criteria:
        ...
        if self.current_direction == 'down' and new_direction == 'up':
            ...
            if self.pitch_peak is not None and self.pitch_valley is not None:
                ...
                if pitch_range >= min_pitch_range and time_since_last_rep >= self.rep_cooldown and is_synchronized_motion:
                    rep_detected = True
                    if self.debug_enabled:
    ...
    def process_imu_sample(self, imu_sample: dict, timestamp: float) -> Optional[Dict]:
        ...
        if self.exercise == 'bicep_curls' and len(self.pitch_buffer) >= 10:
        ...
        if self.debug_enabled:
            ...
            if self.last_debug_time is None or (current_time - self.last_debug_time) >= self.debug_interval:
                ...
                print(f"🔍 Hybrid Detector: buffer={len(self.magnitude_buffer)}, "
                      f"reps={self.rep_count}, last_form_score={form_score_str}")
